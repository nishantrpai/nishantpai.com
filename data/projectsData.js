const projectsData = [
  {
    title: '0xwall',
    description: `Web3 Paywall any link you want with few clicks. No KYC, no deposit, no documents, no charges on any transaction.`,
    imgSrc: 'https://0xwall.app/og.png',
    href: 'https://0xwall.app/',
  },
  {
    title: 'GM Bot',
    description: `A Discord bot that reacts anytime someone says "GM". Includes features like customizing the reaction, and more.`,
    imgSrc: 'https://addgmbot.com/og.png',
    href: 'https://addgmbot.com/',
  },
  {
    title: 'Spell Vault',
    description: `Unlock the secrets of the web with Spellvault - the bot that brings you the most relevant posts for any keyword`,
    imgSrc: 'https://spellvault.app/og.png',
    href: 'https://spellvault.app'
  },
  {
    title: '0xverify',
    description: `Verify if a wallet is scam or not`,
    imgSrc: 'https://0xverify.app/0xverify.png',
    href: 'https://0xverify.app/',
  },
  {
    title: '0xverify discord Bot',
    description: `Verify if a wallet is scam or not in discord. Protect members from scammers.`,
    imgSrc: 'https://0xverify.app/0xverify_discord.png',
    href: 'https://0xverify.app/discord',
  },
  {
    title: 'Tweet2Audio',
    description: `Listen to tweets as audio while you are busy`,
    imgSrc:
      'https://lh3.googleusercontent.com/fVcbuaSC-p9IVYGCtaC-fJB5P0GWb6yR91JfLwmhXl7v_vjIZJ0uO7vaImV-Apzhqz25YqduoS_7iw0dv82NvgeHxw=w640-h400-e365-rj-sc0x00ffffff',
    href: 'https://chrome.google.com/webstore/detail/tweet2audio/naafncepeohchljcfdjckafdhclikcce',
  },
  {
    title: 'Rarity Score',
    description: `Rarity Score is a tool to calculate the rarity of a NFT`,
    imgSrc:
      'https://opengraph.githubassets.com/082e76b287c2665d97f1b38ac5e04d4230080848f770967c1e54c41e87ff7374/nishantrpai/rarity-score',
    href: 'https://github.com/nishantrpai/rarity-score',
  },
  // {
  //   title: 'Privacy Santa (Sold)',
  //   description: `Grow your business online, we'll manage your privacy policy`,
  //   imgSrc: 'https://privacysanta.com/images/preview.jpg',
  //   href: 'https://privacysanta.com',
  // },
  // {
  //   title: 'Logify',
  //   description: `Use Logify for keeping track of context across projects and timestamping events.`,
  //   imgSrc: 'https://logify.vercel.app/logify_og_image.png',
  //   href: 'https://logify.vercel.app/',
  // },
  // {
  //   title: 'Threadify',
  //   description: `Turn your blog into tweets`,
  //   imgSrc: 'https://i.imgur.com/SXjZTze.png',
  //   href: 'https://threadify.vercel.app/',
  // },
  // {
  //   title: 'Covid19 Status',
  //   description: `Track the current status of the COVID-19 and stay up to date about Novel Coronavirus in your location and what you can do to prevent getting infected or spreading of Coronavirus`,
  //   imgSrc: 'https://i.imgur.com/BRgRtmi.png',
  //   href: 'https://literate-octo-journey.vercel.app/',
  // },
  // {
  //   title: 'Youtube Thumbnail',
  //   description: `Generate thumbnail for your Youtube video`,
  //   imgSrc: 'https://i.imgur.com/OtXHhxD.png',
  //   href: 'https://ythumbnail.vercel.app/',
  // },
  // {
  //   title: 'Visualizevalue',
  //   description: `Animates visuals from visualizevalue`,
  //   imgSrc: 'https://i.imgur.com/4vYjJUE.png',
  //   href: 'https://visualizevalue.vercel.app/',
  // },
]

export default projectsData
